...
export function createRequest(body: ToServerSatisfactionTestPermutationRequest, token: string = UUID.generate()) {
    // Create a cancellation token
    // This is used in event of overlapping requests
    const cancelTokenSource = axios.CancelToken.source();

    const request = axios.post(`/api/satisfaction/test-permutation/calculate`, body, {
