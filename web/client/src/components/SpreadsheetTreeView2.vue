...
export default class SpreadsheetTreeView2 extends Vue {
    ...
    get treeMaxDepth() {
        ...
        return this.annealNodeRoots.reduce((carry, node) => {
            return Math.max(carry, this.getMaxChildrenDepth(node));
        }, 0);
    }

    getMaxChildrenDepth(node: AnnealNode.Node, depth = 0): number {
        switch (node.type) {
            case "root":
            case "stratum-stratum":
                return node.children.reduce((carry, child) => {
                    return Math.max(carry, this.getMaxChildrenDepth(child, depth + 1));
                }, depth + 1);
    ...
    get totalNumberOfColumns() {
        return this.headerRow.length + this.treeMaxDepth;
    }
